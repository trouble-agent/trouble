...
func (s *Sensors) startDisk(ctx context.Context) error {
	...
	// No non-Linux disk implementation exists: report the platform gap instead
	// of sweeping mounts through a stub that would read as 0%% free
	// (SPEC-03 §3.3, SPEC-12 §3.5a).
	if reason := diskPlatformReason(); reason != "" {
		s.setSensor(types.SenDisk, false, true, reason)
		return nil
	}
