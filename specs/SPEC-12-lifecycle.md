...
ACs: AC-14, AC-18, AC-25, AC-26, AC-27, AC-28, AC-29, AC-33, AC-34
PRD: §09, §11, §12
...
AC coverage: **AC-14/AC-18** are exercised by the bind matrix, the forward path and the spool (§3.5, §3.7); **AC-25** by the satellite forward + pull-down mechanics (§3.7); **AC-26** by the kill-switch persistence, the autonomy record and the watchdog chain (§3.2, §3.6); **AC-33** by the `flow.*` bound resolution (§3.1d); **AC-34** by the §3.3a Status rule the health assembly enforces (§3.3a). Sections no AC reaches are **design constraints**: the config precedence and `explain` contract (§3.3), the unit/sandbox decisions (§2, §3.1), the upgrade park/resume rules (§3.6) and the refusal classes (§5, §6) are obligations of this subsystem rather than user-visible acceptance criteria — the loop records them as constraints per SPEC-INDEX §7 step 2.
...
### 3.1d The `[flow]` table: the SPEC-08 §3.9a dispatch queue's bounds (AC-33)
...
### 3.3a Per-subsystem built/refused block (`subsystems[]`) — the Status rule (AC-34)

**Acceptance criterion (SPEC-INDEX §3.2).** **AC-34** binds the Status rule this section pins: over all
32 built-combinations of the five rows, `status="ok"` is reachable ONLY on the all-built set — any row
with `built=false` degrades the response, whichever row it is and in either shape (refused with its code,
or unbuilt with no refusal recorded), and the reverse is not claimed (an all-built response may still
degrade through a sensor, an unstamped build or a degraded hub tier). Enforced by
`internal/lifecycle/health_test.go` and `internal/app/health_wiring_test.go` (§7).
...
|---|---|---|
| `internal/lifecycle/health_test.go` | **AC-34** (§3.3a): the Status rule over the subsystem block — an exhaustive sweep of all 32 built-combinations of the five rows asserting exactly one (the all-built set) reads `ok` and every set with a `built=false` row reads `degraded`; a per-row table over all five names in BOTH shapes (refused carrying TROUBLE-LIFECYCLE-001, unbuilt with no refusal) asserting the degradation and the row's own detail keys (`subsystem_refused` + `subsystem`, or `subsystem_unbuilt`); plus the pre-existing spot rules the block keeps — all-built `ok` with the 5 rows intact and no refusal key, a refused sentinel degrading with its code, a code-less refusal carrying its reason, an unbuilt row reported as unbuilt and never as refused, `stalled` precedence over the subsystem degrade with the code still in `detail`, the last-refused-row determinism of the single-valued keys, and the kept `unstamped_build` detail | 32/32 combinations classified with exactly one `ok`; 5 names × 2 shapes degraded with THAT row named (10/10); reverting the §3.3a subsystem loop in `health.go` reds the sweep at `mask 0b0 … reads status=ok while a row is built=false`; the spot rules stay green |
| `internal/app/health_wiring_test.go` | **AC-34** at the composition root: `Subsystems.Report()` over a COMPLETE set (every row built=true refused=false), then one member dropped nil at a time over all five names — the affected row and ONLY that row loses `built`, the block keeps all five rows in build order; the same call the dashboard makes — `Report()` feeding `lifecycle.Health` — composes `ok` on the complete set and `degraded` with `detail.subsystem_unbuilt` naming the missing member for each of the five partial sets | 5/5 partial sets honest row-by-row and in order; the composed seam reads `ok` exactly once (complete set) and `degraded` on all five partials |
...
| `internal/app/readybudget_test.go` | the boot-budget derivation over synthetic host figures (monotone in load, never below the base, capped at 4x, exactly the base with no host signal, a core count below 1 coerced to one core, a measured I/O tier below the reference clamped to 1 and composed into the term), a load×tier sweep asserting the monotonicity, the cap and the one-way rule as properties — the host term is never TIGHTER than the load-only term it replaced (TRBL-055), and the 2026-09-18 quiet-box case (9,747 ms against a 9,000 ms budget) is no longer graded at 1.00x — the once-per-process pilot assertion (`sync.OnceValue`, both multiples sane, the tier clamped), the operator pin (`TROUBLE_HOST_CALIB`), the kernel-signal read, the resolution of the process's own host term (`hostTermFor`), and the verdict composer: the failure text, the SKIP text, the threshold just below the fence, the drain wording, and the composed message for a forced low budget | every case asserted; both texts carry the measured I/O tier, the observed `load_avg` and the elapsed seconds; 0 SKIPs below 4 runnable threads per core; the pilots run exactly once per test process; the host term is ≥ the load-only term it replaces at every swept load; the quiet slow box from the filing incident is no longer graded at 1.00x |
| `internal/app/bootphase_test.go` | a real boot with `BootOptions.OnBootPhase` attached: the observed §4.1 phase sequence equals `BootPhaseNames()` in order with no name missing or repeated, the marks are monotone, the last phase is `ready`, and the per-phase table is printed with the ledger's record/commit accounting; on a host whose boot writes ≥20 records the §3.3a batch must be engaged — durable commits strictly below the record count (a record-count commit count means the boot is paying one window per record again) | the table is complete and ordered (an unattributed phase fails); the batch accounting is asserted with the measured numbers logged (TRBL-044 collapsed the reconcile's N windows; SPEC-12 §7b re-derived); on a record-poor host the causal assertion is skipped and says why |
...
| `internal/ledger/batchappend_test.go` | the §3.5a batch contract: one `AppendBatch` of K records = exactly 1 durable commit while the same K appended sequentially cost K on the same ledger (the §3.5 serial model, asserted as the control arm); batch seqs contiguous and in draft order with every per-record field stamped; `LastSeq` = the batch's last seq and the persisted lines are the batch's seqs in order; an empty batch writes nothing and one bad draft fails the call with 0 records written (never a half batch); K > `max_batch_records` splits into `ceil(K/cap)` commits with one contiguous seq range | 1 batch of 10 = 1 commit, control arm = 10 commits (measured, not assumed); 0 partial batches; the split is invisible to seq order |
| `internal/sensors/dbus_batch_test.go` | the §3.3a batch reconcile: a sweep's decision phase drafts one record per failed unit (the per-unit model) and the write phase calls the batch boundary exactly once with all of them; every batch member carries `arrival_path = reconcile` and its per-unit merge key; a failed batch drops the WHOLE sweep — 0 sweep records written, drop count = the whole sweep, and a TROUBLE-SENSORS-015 record naming `reconcile batch write`; without a batch boundary the sweep falls back to per-record emits; a firing member reaches the ladder bridge only AFTER the batch returned, with the durable record's real rec_id; a second sweep inside the merge window attaches (count=2) in its own single batch | 50 units → 1 batch call carrying 50 drafts; 0 partial sweeps; the §3.6 merge rule decides membership, not the batch |
| `internal/app/bootbatch_test.go` | the composition root's §3.3a wiring on a real boot: the daemon installs the batch boundary (`AppendBatch` over its own ledger) and it is reachable on the `Daemon`; a batch through the installed boundary lands durably (watermark advances by K) and costs exactly 1 durable commit for K=6 | the boundary is installed on every boot and is the ledger's own `AppendBatch` (not a second write path); 1 commit for a 6-record batch |
...
The boot tests in `internal/app` wait on a live boot (`daemon_test.go`, `subsystems_config_test.go`, `shipped_example_test.go`, `config_projects_test.go`). That boot measures ~22-25 s on a quiet 16-core host, so a bare wall-clock deadline turns host load into a verdict: the identical commit passes on an idle box and reds out while the same box carries fleet load. Every host-measured wait in that package is therefore derived from the host's own measurement — its measured speed and the load it is running under — and a budget only ever bounds "still booting":

- **Derivation.** `base × clamp(ioTier × (1 + loadPerCore), 1, 4)`, with `loadPerCore = loadAvg1 / NumCPU()` and `ioTier` this host's measured I/O multiple (SPEC-01 §7a). `loadAvg1` is field 0 of `/proc/loadavg`, and 0 on any read or parse error; 0 keeps the base budget and never selects the extreme-load verdict. **Load alone is not a host signal, and the load-only form of this rule was wrong (TRBL-055)**: `load_avg` says how BUSY a box is, never how FAST it is, so a multiplier built from it grades a QUIET SLOW box more strictly than a BUSY FAST one — the inversion SPEC-01 §7a was written against, where the same commit rebuilt the same 512 MiB ledger fixture in 9,747 ms against a 9,000 ms reference budget on a clean JIT box at load_avg 3.99 (FAIL) while a busier box at load_avg 10.78 was graded against 30,000 ms for identical work (PASS). The **I/O tier**, not the CPU tier, is the right speed term for these waits, and §7b is the reason: ~92 % of a quiet boot is spent inside ledger group-commit windows (`records × (window + device sync latency)`), so what stretches a boot is the block device's sync latency — the thing the I/O pilot prices. A CPU-bound gate in this package would use `CPUScale()` instead; mixing the two terms is the mistake this rule exists to prevent. The two halves compose: `L` other runnable threads per core stretch the boot by ~`1 + L` (slope 1, not 1/2, because §7b measured what the stretch is), and the tier carries the box's own measured throughput. The host term never drops below 1 (a quiet reference-class host never tightens a budget) and never exceeds 4 — a ceiling reached at three runnable threads per core, so a budget at the fence is already at its maximum. On a quiet reference-class host the tier is exactly 1 and the budgets are the shipped spec numbers to the millisecond; on any other host they are ≥ what the load-only rule allowed, because calibration may only ever LOOSEN a budget relative to the reference numbers — a budget the reference host meets must stay meetable here.
- **Measured once per test process.** The hosts' two pilots — write+read 8 MiB with 16 fsyncs, and the fixed integer workload — run ONCE per test process through `sync.OnceValue`, and one `bootHostTerm` serves the budget, the log line and the verdict text of each wait, so the scale a failure prints is always the arithmetic of the speed and load printed beside it. Re-reading the host at expiry, or re-running the pilots per assertion, would print a figure that no longer explains the budget. `TROUBLE_HOST_CALIB` pins the multiples in place of the pilots (the same hook SPEC-01 §7a defines), so an operator override still wins over automatic calibration.
...
- **An exhausted budget names its numbers.** The failure carries the observed `load_avg`, the measured I/O tier, the elapsed seconds, the base and the derived budget in one message, so a reader can tell host speed and host load from a regression.
...
**The decision (TRBL-044, and the re-derived cost model).** The SPEC-01 §3.5a / SPEC-03 §3.3a decision is the
**batch-append path**: the startup reconcile's records land through `AppendBatch` — K records, ONE group
commit, durable-on-return per BATCH — and the reconcile stays on the READY path. The off-path alternative
was rejected: READY-first + background reconcile would blind the §3.3 incident model for up to one
reconcile interval on every boot to save the same windows, a worse trade than §3.5a's bounded crash tail.
The cost model changes accordingly: the boot's durability term is no longer
`records × (window + device sync latency)` but roughly `windows × (window + latency)`, with
windows = ceil(N / max_batch_records) — for a boot writing ~90 reconcile records that is ~1 window
instead of ~90. The boot-phase test now asserts the batching is engaged (durable commits strictly below
the record count on a record-rich boot) instead of asserting `sensors_start` is the widest phase: the
old assertion pinned the disease. Measured on the same host class as the table above, after the change
and under comparable fleet load: `sensors_start` collapsed from one-window-per-record arithmetic to a
handful of batch commits (98 records in 28 durable commits, of which the reconcile's share is a small
number of batches), and the pre-fix cost model would have priced the same loaded boot at roughly 4× the
measured wall. The §7a multiplier is unchanged: it bounds the host term, which the batch does not touch.
