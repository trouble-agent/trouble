...
type RedisStreamOffsets struct {        // live queue state; the zero value when profile=standalone
    ...
    DedupRestored   int64  `json:"dedup_restored"`  // size of the ledger idempotency index the gate was re-warmed with (SPEC-13 §2.1.1 rule 5a); 0 = window not warm
