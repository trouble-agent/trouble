...
Local types: — 
ACs: AC-1..AC-34 (matrix owner; every AC appears exactly once in the matrix, with its spec mapping)
PRD: §10, §11, §12
...
| SPEC-12 | lifecycle: unit, watchdog, upgrades, config, topology, server profiles | §09, §11, §12 | AC-14, AC-18, AC-25 (partial), AC-26, AC-27, AC-28, AC-29, AC-33, AC-34 | partial |
...
| AC-34 | AC-34 Health status impossibility (SPEC-12 §3.3a, pinned): `status="ok"` is IMPOSSIBLE while any `subsystems[]` row reports `built=false` — over all 32 built-combinations of the five rows (`sentinel`, `issues`, `research`, `flow`, `skills`), exactly one, the all-built set, may read `ok`, and every set carrying at least one `built=false` row reads `degraded` (whichever row it is, in either shape — a refusal carrying its `TROUBLE-*-NNN` code or an unbuilt row with no refusal recorded — with `detail.subsystem_refused` + `detail.subsystem` or `detail.subsystem_unbuilt` naming THAT row), and `stalled` keeps its §3.3 precedence over both; the reverse is NOT claimed — all five `built=true` rows do not by themselves force `ok` (a degraded sensor, an unstamped build or a degraded hub tier still degrades an all-built response). The rule holds at the composition root too: the block the health surface reads is the live subsystem set's own report, a partial set (one member of an otherwise-live set absent) degrades the composed response, the block is never omitted or shortened (an unassembled set reports five not-built rows, never fewer), and the rows stay in build order. | SPEC-12 §3.3a | B |
...
| AC-34 (allocated for SPEC-12 §3.3a) | none — the per-subsystem health block and the status rule are shipped behaviour (`internal/lifecycle/health.go`, `internal/app/subsystems.go`, `dashdeps.go`), all inside the §3.3 cut line, and no deferred item is a passing condition | Status `B`: the §3.3a Status rule (no `ok` with a `built=false` row), the never-omitted block, the per-row detail keys and the build ordering are existing behaviour with named enforcing tests (SPEC-12 §7 rows `internal/lifecycle/health_test.go` + `internal/app/health_wiring_test.go`; `health_test.go` added the exhaustive 32-combination sweep and the all-five-names detail table that had no test — the spot tests held only sentinel/issues/skills). |
...
- Every spec file is a new node under `~/trouble/specs/`; `specs/tools/selfcheck.py` is the
  ...
  `AC-1..AC-34` as of the AC-34 allocation for SPEC-12 §3.3a, derived from §3.2 rather than
