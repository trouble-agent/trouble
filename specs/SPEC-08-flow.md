...
ACs: AC-9, AC-19, AC-21, AC-22, AC-26, AC-32, AC-33
PRD: §08, §11, §12
...
**Acceptance criteria (SPEC-INDEX §3.2).** **AC-32** binds the claim this section makes: a dispatch
that cannot be delivered is either durably queued on a queue this subsystem replays and replayed with
the same `idem_key`, or refused as `unspooled`/`spool_failed` naming the coupling — never a `spooled`
record a loop cannot honour. **AC-33** binds the bounds table below: the bound in force is the
resolved `flow.*` key on both halves, and every entry a bound costs is a `flow` + `spawn` drop pair
...
| `dedup_test.go` | 1 000 sigs filed concurrently; same sig ×3 incidents; same sig on two boards (shared vs unshared repo); missing-row comment → FLOW-019 | exactly 1 000 rows; 0 duplicates; 3 comments on the recurring row; FLOW-019 recorded once with the intended id. The 1,000-row fixture is **host-calibrated per SPEC-01 §7a** (`loadfence.Measure(dir).Scale()` over the board's own filesystem): the spec shape is asserted unchanged on a quiet reference-class host — and wherever /proc/loadavg is unavailable, which grades as load 0 — while a contended host or one with slower measured I/O files a proportionally smaller fixture (1000/scale rows, floor 200) so the volume of fsync'd durability work follows the box's own measured speed and load, never a bar tighter than the reference host's |
...
| `spool_test.go` | **AC-32**/**AC-33** (§3.9a): an `Enqueue`-only sink (the desk adapter's shape) yields EXACTLY ONE outcome — an honest `unspooled` record naming the coupling — and zero `spooled` claims; a typed-nil store is not adopted; with the flow's own store a parked spawn is one `0600` file under `<state_root>/spool/flow/spawn/` and the record says `spooled`; a drain re-dispatches with the SAME idem key against a real HTTP router, deletes the entry on success, re-drains to zero, and leaves an empty queue after a simulated restart; an entry written by one store instance is listed, decoded and replayed by a second one; `attempts`, `ttl`, `corrupt` and overflow each drop the entry with a `flow` + `spawn` record pair and zero `gap` records; overflow evicts the OLDEST entry, never the incoming one; `Flow.Run` drains the queue on its ticker. `flow_spool_wiring_test.go` (internal/app, **AC-32** at the composition root) drives the REAL `flowDeps` with the issue desk OFF (the shipped posture): the flow still receives a replayable queue, the wired sink writes one `0600` entry under the state root, and a store that cannot be built leaves the flow reporting "no queue" | 0 `spooled` records while no replayable queue is wired; exactly 1 file per entry, mode `0600`, directory `0700`; queue depth 0 after a successful replay and 0 after the restart; 2 overflow drops for 4 writes at a bound of 2; the loop test's deadline is generous by design (wall-clock driven), every other assertion is clock-injected. **AC-32 enforced by** `TestDispatchIsRefusedHonestlyWhenTheSinkIsNotReplayable`, `TestDispatchIsDurablyQueuedOnTheFlowOwnedStore`, `TestDispatchIsRecordedAsSpoolFailedWhenTheStoreRefusesTheWrite`, `TestFlowDoesNotAdoptTheDeskSpool`, `TestFlowDoesNotAdoptATypedNilStore`, `TestReplayRefusesHonestlyWithNoQueue`, `TestFlowOwnedSpoolReplayRedispatchesWithTheSameIdemKey`, `TestSpoolSurvivesRestartAndReplaysAfterIt`, `TestReplayHoldsOneInFlightDispatchPerIdemKey`, `TestReplayAcceptsA409DuplicateOnReplay`, `TestFlowSpoolSkipsForeignAndTornFiles` (internal/flow) and `TestFlowDepsWiresAQueueTheFlowCanReplayWithTheDeskOff` / `TestFlowDepsWiresNoQueueWhenTheStoreCannotBeBuilt` (internal/app); **AC-33 enforced by** `TestFlowSpoolBoundsDropOldestWithALedgerNote`, `TestReplayDropsAfterTheAttemptBudgetLeavesNothingSilent`, `TestReplayDropsAnExpiredEntry`, `TestReplayDropsACorruptEntryInsteadOfLoopingForever`, `TestReplayDueIsFalseOnAnEmptyOrBackedOffQueue`, `TestFlowRunDrainsTheQueueAndProbes` |
...
| `internal/app/flow_bounds_wiring_test.go` | **AC-33**: the §3.9a bounds as resolved SPEC-12 §3.1d keys at the composition root: `flowSpoolBounds` over a config file with `[flow] spool_max_entries = 3` carries 3 and the §3.9a defaults for the four undeclared keys; four `Put`s through `newFlowSpool` leave three entries, report exactly one `overflow` drop for the OLDEST id and leave it off disk — where the same four writes with no `[flow]` table leave four; a two-hour-old, undecodable entry is dropped with `drop_reason:"ttl"` through `newFlowSubsystem` + `Replay` under `flow.spool_ttl = 1h`, and never with `"ttl"` under the compiled default | 2 eviction cases (configured 3 → depth 3 + oldest gone; default → depth 4); the TTL case is a 2-row table whose control row must NOT produce a `ttl` drop; reverting either the projection or the flow's construction fails the suite. Enforced by `TestFlowQueueEvictsAtTheConfiguredBoundNotAtTheConstant` and `TestFlowReplayUsesTheConfiguredTTLNotTheConstant` |
