...
(The numbered list below is 1-8 the §3 test surfaces and 9-12 cross-spec test entries; §5's
numbered list is the gate obligations, not the test roster — QA-TROUBLE-5 disambiguation.)
...
3. `verify_test.go` — the fixture verifies with the pinned public key/signature; wrong key → 002; unknown `signer_key_id` → 003; `enabled=false` → 003; `trust=local` key on a pulled artifact → 003; truncated/space-padded base64 → 002. Budget: 100 artifacts (parse + canonicalize + ed25519 verify) ≤ 200 ms **× `CPUScale()`** (host calibration per SPEC-01 §7a — the spec number is a reference-host measurement, so a slower box is graded against its own measured speed, never a tighter bar than the reference host's); ed25519 verify alone ≤ 1 ms per artifact.
